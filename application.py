...
def homestaff():
    if request.method == "GET":
        return render_template("index.html",msg = "home_staff",name=session["user_username"])
    else:
        course = request.form.get("add_course")
        ass = request.form.get("add_assignment")
        person = listify(db.execute("SELECT * FROM teachingStaff WHERE staffId = :id",{"id":session["user_id"]}) )
        if course is not None:
            courses = listify(db.execute("SELECT * FROM Courses WHERE department IN ( :department , 'asu' )",{"department":person[0]['department']  }) )
            return render_template("index.html",msg = "courses_section",courses = courses)
        else:
            return apology("assignment selected")
...
def homestudent():
    if request.method == "GET":
        return render_template("index.html",msg = "home_student",name=session["user_username"])
...
def register_staff():
    if request.method == "GET":
        ...
        if len(username) < 5:
            return apology("your Account should be at least 5 characters long")
        users =  listify(db.execute("SELECT * FROM teachingStaff WHERE username =:username", {"username":username}))
        if len(users) != 0:
            return apology('Email Already Exists!')
        if degree == "":
        ...
        if  confirm != password:
            ...
            db.execute('INSERT INTO teachingStaff (name,degree,username,password,department) VALUES(:name,:degree,:username,:password,:department)',{"name":fullname,"degree":degree,"username":username,"password":generate_password_hash(password),"department":dept } )
            con.commit()
            #Redirect the user to home page
        return redirect("/login")
...
def login():
    ...
    if request.method == "POST":
        ...
        if Type == 'student':
            ...
            rows = listify(db.execute("SELECT * FROM teachingStaff WHERE username = :username",{"username":request.form.get("username")}) )
            # Remember which user has logged in
